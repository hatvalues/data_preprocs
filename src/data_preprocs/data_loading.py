...
def is_polar_schema(arg: dict) -> bool:
    if not isinstance(arg, dict):
        return False
    return all(
        isinstance(k, str) and isinstance(v, (pl.DataTypeClass, pl.DataType))
...
class DataLoader:
    ...
    def load(self) -> None:
        if validate_framework(self.data_framework, self.schema) == "pandas":
            ...
            target = pd.Series(data[self.class_col]),
            features = data.drop(columns=self.class_col, axis=1)
            )
        else:
            data = load_data_file_to_polars(self.file_name, self.schema)
            self.container = DataContainer(
            target = data[self.class_col],
            features = data.drop(self.class_col)
            )
